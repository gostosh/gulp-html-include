...
module.exports = function(opts) {
  ...
  function fileInclude(file, enc, cb) {
    if (file.isNull()) {
      ...
      //    new gutil.PluginError(PLUGIN_NAME, "Streams are not supported!"));
      ...
      try {
        ...
        var content = cheerio.load(file.contents.toString("utf8"));
        var data = '';
        content(opts.selector).each(function (i, el) {
          data += content(this).html();
        });

        file = include(file, data);
        cb(null, file);
      } catch (e) {
        cb(new gutil.PluginError(PLUGIN_NAME, e.message));
      }
